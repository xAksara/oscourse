...
load_icode(struct Env *env, uint8_t *binary, size_t size) {
    ...
    //     if (ph->p_type != ELF_PROG_LOAD)
    //         continue;
	// void *seg_start = binary + ph->p_offset;
	// void *seg_va = (void *)ph->p_va;
    //     if (ph->p_filesz > ph->p_memsz)
	//     continue;
    //     memcpy(seg_va, seg_start, ph->p_filesz);
    // 	memset(seg_va + ph->p_filesz, 0, ph->p_memsz - ph->p_filesz);
    //     bind_functions(env, binary, size, (uintptr_t)seg_va, (uintptr_t)seg_va + ph->p_filesz);    
