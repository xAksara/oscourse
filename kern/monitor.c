...
int mon_lab2(int argc, char **argv, struct Trapframe*tf);
...
// LAB 2: random text
int
mon_lab2(int argc, char **argv, struct Trapframe *tf) {
    cprintf("Ya ustal\n");
    return 0;
}
...
mon_dumpcmos(int argc, char **argv, struct Trapframe *tf) {
    ...
    cprintf("%02x: %02x", 0, cmos_read8(0));
    for (int i = 1; i < 128; i++) {
        if (!(i % 16)) {
            cprintf("\n%02x: %02x", i, cmos_read8(i));
        }
        else cprintf(" %02x", cmos_read8(i));
    }
    cprintf("\n");
