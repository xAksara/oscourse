...
alloc_block(void) {
    ...
    if (blockno == 0) {
        int first_nonzero_64bit = 1;
        for (; first_nonzero_64bit < super->s_nblocks / 64; first_nonzero_64bit++)
            if (((uint64_t *)bitmap)[first_nonzero_64bit] != 0)
                break;
        for (uint64_t i = first_nonzero_64bit * 64; i < super->s_nblocks; i++)
            if (block_is_free(i)) {
                blockno = i;
                break;
            }
